...
export function BriefingForm({ project, onCancel, onSuccess }: { project: any, onCancel?: () => void, onSuccess?: () => void }) {
    ...
    const form = useForm<OnboardingFormValues>({
        ...
        defaultValues: {
            ...
            funnelCount: project.funnelCount || 1,
            segment: project.client.segment || "",
            operationSize: (project.client.operationSize as any) || "Pequena",
            projectType: (project.projectType as any) || "Corrente",
            technicalBriefingUrl: project.technicalBriefingUrl || "",
        },
    })

    // Flowchart State
    const [nodes, setNodes, onNodesChange] = useNodesState(project.briefing?.flowchartData?.nodes || initialNodes)
    const [edges, setEdges, onEdgesChange] = useEdgesState(project.briefing?.flowchartData?.edges || initialEdges)
    const [selectedNodeId, setSelectedNodeId] = useState<string | null>(null)
    const [nodeName, setNodeName] = useState("")

    // Helper to update group dimensions
    ...
    function onSubmit(data: OnboardingFormValues) {
        startTransition(async () => {
            ...
            const result = await saveBriefing(project.id, fullData)
            if (result?.error) {
                alert(result.error)
            } else if (onSuccess) {
                onSuccess()
        ...
        <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
                ...
                <div className="flex justify-end gap-2 pb-12">
                    ...
                    <Button type="submit" size="lg" disabled={isPending} className="gap-2 min-w-[200px]">
